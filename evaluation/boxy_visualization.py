...
def parse_args():
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="evaluation/boxy_visualizations",
    ...
    parser.add_argument(
        "--img_width", type=int, default=1232, help="Original image width"
    )
    parser.add_argument(
        "--img_height", type=int, default=1028, help="Original image height"
    )
    parser.add_argument(
        "--show_examples",
        type=int,
...
def create_heatmap(annotations, output_path, img_width, img_height, resolution=400):
    ...
    plt.imshow(
        heatmap,
        cmap="viridis",
        norm=LogNorm(vmin=min_val, vmax=max(min_val + 1, max_value)),
    )
...
def find_suspicious_annotations(
    annotations, output_dir, region_params=None, sample_count=5
):
    """
    ...
    for region in region_params:
        ...
        suspicious_anns = [
            ...
            if (
                region.get("class_id", None) is None
                or ann["class_id"] == region["class_id"]
            )
            and region["x_min"] <= ann["x_center"] <= region["x_max"]
            and region["y_min"] <= ann["y_center"] <= region["y_max"]
        ]
        ...
        for img_path in tqdm(
            sampled_images, desc=f"Visualizing '{region_name}' region"
        ):
            # Read image
            img = cv2.imread(img_path)
            if img is None:
            ...
            all_img_annotations = [
                ann for ann in annotations if ann["img_path"] == img_path
            ]
            ...
            for ann in all_img_annotations:
                ...
                if is_suspicious:
                    ...
                    cv2.drawMarker(
                        img, (center_x, center_y), color, cv2.MARKER_CROSS, 20, 2
                    )
            ...
            cv2.rectangle(
                img, (region_x1, region_y1), (region_x2, region_y2), (0, 255, 255), 2
            )  # Yellow boundary
...
def main():
    ...
    train_annotations = load_annotations(
        args.data_dir, subset="train", sample_size=args.sample_size
    )
    ...
    visualize_class_distribution(
        train_annotations, args.output_dir, args.img_width, args.img_height
    )

    # Visualize example images
    print("Creating example image visualizations...")
    visualize_example_images(
        train_annotations, args.output_dir, sample_count=args.show_examples
    )
