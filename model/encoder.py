# ====================================
# InputEncoder Implementierungen
# ====================================
...
class SimpleInputEncoder(InputEncoder):
    """
    Einfacher Encoder ohne komplexe Attention-Mechanismen.

    Kodiert Telemetriedaten und Objektdetektionen unabhängig und aggregiert
    die Detektionen durch Mittelwertbildung über alle gültigen Objekte.

    Args:
        telemetry_input_dim (int): Dimensionalität der Telemetriedaten
        detection_input_dim_per_box (int): Dimensionalität der Eigenschaften einer Objektdetektion
        embedding_dim (int): Dimensionalität der erzeugten Embeddings
        dropout_prob (float): Dropout-Wahrscheinlichkeit
        use_positional_encoding (bool): Ob Positions-Encoding verwendet werden soll
        max_seq_length (int): Maximale Sequenzlänge für Positions-Encoding
    """
    ...
    def __init__(
        ...
        # Detektions-Embedding (pro Objekt)
    ...
    def _create_position_encoding(self, max_len, d_model):
        """Erstellt Positions-Encoding für Sequenzen nach Transformers-Paper."""
        ...
        pe = pe.unsqueeze(0)  # Batch-Dimension hinzufügen
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        Forward-Pass durch den SimpleInputEncoder.
        ...
        Returns:
            dict: Dictionary mit encodierten Features
        ...
        # Positional Encoding hinzufügen, falls aktiviert
        ...
        # --- Detektions-Encoding ---
        # Reshape für effiziente Verarbeitung
...
class AttentionInputEncoder(InputEncoder):
    """
    Erweiterter Encoder mit interner Self-Attention für beide Modalitäten.

    Verwendet Self-Attention, um Beziehungen zwischen Objekten sowie
    zeitliche Muster in den Telemetriedaten zu erfassen.

    Args:
        telemetry_input_dim (int): Dimensionalität der Telemetriedaten
        detection_input_dim_per_box (int): Dimensionalität der Eigenschaften einer Objektdetektion
        embedding_dim (int): Dimensionalität der erzeugten Embeddings
        attention_num_heads (int): Anzahl der Attention-Heads
        dropout_prob (float): Dropout-Wahrscheinlichkeit
        max_detections (int): Maximale Anzahl von Detektionen pro Zeitschritt
        max_seq_length (int): Maximale Sequenzlänge für Positions-Encoding
    """
    ...
    def __init__(
        ...
        if embedding_dim % attention_num_heads != 0:
            raise ValueError(
                ...
                f"attention_num_heads ({attention_num_heads}) teilbar sein"
            )
        ...
        # Temporal Self-Attention für Telemetrie
        ...
        # Object-to-Object Self-Attention für Detektionen
        ...
        # Layernorms für Residual-Verbindungen
        self.norm_telemetry = nn.LayerNorm(embedding_dim)
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        Forward-Pass durch den AttentionInputEncoder.
        ...
        Returns:
            dict: Dictionary mit encodierten Features
                "telemetry_features": (batch_size, seq_len, embedding_dim)
                "detection_features": (batch_size, seq_len, embedding_dim)
                "attention_weights": Optional - Dictionary mit Attention Weights
        """
        batch_size, seq_len, _ = telemetry_seq.shape
        _, _, max_dets, det_feat_dim = detection_seq.shape
        ...
        # --- Telemetrie Self-Attention (temporal) ---
        ...
        # --- Object-to-Object Self-Attention (pro Zeitschritt) ---
        objects_attended = []
        object_attn_weights = []

        for t in range(seq_len):
            # Extrahiere Objekte für diesen Zeitschritt
            objects_t = embedded_detections[
                :, t
            ]  # (batch_size, max_dets, embedding_dim)
            mask_t = detection_mask[:, t]  # (batch_size, max_dets)

            # Erstelle Attention-Maske (True = ignorieren)
            obj_attn_mask = ~mask_t

            # Transposition für Attention: (max_dets, batch_size, embedding_dim)
            ...
            # Zurück transponieren: (batch_size, max_dets, embedding_dim)
