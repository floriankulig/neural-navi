...
class SingleArchitectureTrainer:
    ...
    def setup_model(self):
        ...
        filtered_weights = {
            task: weight for task, weight in task_weights.items() 
    ...
    def train_epoch(self, epoch: int):
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            for task_name in self.config["prediction_tasks"]:
                targets[task_name] = batch["targets"][task_name].to(self.device, non_blocking=True)
    ...
    def train(self):
        ...
        for epoch in range(self.config["epochs"]):
            ...
            self.logger.info(f"   ⌛ Patience: {self.patience_counter}/{self.config['patience']}")
        ...
        with open(logs_path, "w") as f:
            ...
            for entry in self.training_history:
                serializable_entry = {
                    ...
                    "train_losses": {k: float(v) for k, v in entry["train_losses"].items()},
